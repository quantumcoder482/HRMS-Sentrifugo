...
class Default_Model_Monthlypayroll extends Zend_Db_Table_Abstract
{
    ...
    public function getMonthlypayrollData($sort, $by, $pageNo, $perPage,$searchQuery,$userid,$usergroup,$req_type,$filter='')
    {
...
//            $where .= " AND r.createdby = ".$userid." ";
        ...
        $roleData = $this->select()
            ...
            ->from(array('r'=>"main_monthlypayrolls"),array('id'=>'id','employee_id','starting_date','employee_name','department','comments',
                ...
                'whtaxpaided_salary','progresive_whtax','bankpaid_salary','whtax_salary'
            ))
            ->where($where)
            ->order("$by $sort")
    ...
    public function getDepartmentCount($req_type)
    {
...
//			$where .= " AND r.createdby = ".$userid." ";
	...
	public function getGrid($sort,$by,$perPage,$pageNo,$searchData,$call,$loginUserId,$loginuserGroup,$reqtype,$objName,$dashboardcall)
	{
		...
		$tablecontent = $this->getMonthlypayrollData($sort, $by, $pageNo, $perPage,$searchQuery,$loginUserId,$loginuserGroup,$reqtype);
